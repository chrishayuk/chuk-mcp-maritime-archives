...
# Run the MCP server
run:
	@echo "Running Maritime Archives MCP server..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_maritime_archives.server; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_maritime_archives.server; \
	fi
...
docker-build:
	...
	docker build -t chuk-mcp-maritime-archives:latest .
	@echo "Docker image built: chuk-mcp-maritime-archives:latest"

# Run Docker container
docker-run:
	@echo "Running Docker container..."
	docker run --rm -p 8005:8005 chuk-mcp-maritime-archives:latest
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-maritime-archives==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-maritime-archives==$$version"
