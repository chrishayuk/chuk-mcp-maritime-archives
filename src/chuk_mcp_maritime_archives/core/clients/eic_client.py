"""
English East India Company (EIC) client for voyages and wrecks.

Reads locally stored JSON data produced by scripts/generate_eic.py.
Covers EIC voyages between England and Asia, 1600-1874.

Source data compiled from:
    Hardy, Charles. A Register of Ships. London, 1835.
    Farrington, Anthony. Catalogue of EIC Ships' Journals. London, 1999.
"""
...
class EICClient(BaseArchiveClient):
    """
    Client for the English East India Company voyage database.

    Searches and retrieves records from locally cached JSON files
    containing curated EIC voyage records, 1600-1874.
    """

    VOYAGES_FILE = "eic_voyages.json"
    WRECKS_FILE = "eic_wrecks.json"
    ...
    async def search(
        ...
        *,
        ...
        route: str | None = None,
        fate: str | None = None,
        company_division: str | None = None,
        max_results: int = 50,
        **kwargs: Any,
    ) -> list[dict]:
        """Search EIC voyage records from local data."""
        ...
        if company_division:
            results = [
                v for v in results if self._contains(v.get("company_division"), company_division)
            ]
        if date_range:
    ...
    async def get_by_id(self, record_id: str) -> dict | None:
        """Retrieve a single voyage by its EIC voyage ID."""
        index = self._get_voyage_index()
        if record_id in index:
            return index[record_id]
        prefixed = f"eic:{record_id}" if not record_id.startswith("eic:") else record_id
    ...
    async def search_wrecks(
        ...
        **kwargs: Any,
        ...
        """Search EIC wreck records from local data."""
    ...
    async def get_wreck_by_id(self, wreck_id: str) -> dict | None:
        ...
        prefixed = f"eic_wreck:{wreck_id}" if not wreck_id.startswith("eic_wreck:") else wreck_id
        return index.get(prefixed)
